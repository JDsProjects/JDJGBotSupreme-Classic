...
async def servers(ctx):
    if await client.is_owner(ctx.author):
        send_list = [""]
        guild_list = [
            "%d %s %d %s" % (len(g.members), g.name, g.id, (g.system_channel or g.text_channels[0]).mention)
...
async def on_message(message):
    ...
    if message.reference != None and not message.content.startswith(discordprefix) and not message.author.bot:
        if message.mentions != None and client.user in message.mentions:
            ...
            await client.get_channel(996864571962839143).send(
                content=f"{jdjg.mention} {message} \n Content: {message.content} "
            )
    ...
    if (
        message.content.startswith(discordprefix)
        and mention == False
        and not message.author.bot
        and test.valid is False
    ):
        pfp = message.author.display_avatar.url
        time_used = (message.created_at).strftime("%m/%d/%Y %H:%M:%S")
        embed_message = discord.Embed(
            title=f" {message.content}", description=time_used, color=random.randint(0, 16777215)
        )
