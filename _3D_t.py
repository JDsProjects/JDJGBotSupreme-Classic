...
class tri:
    def __init__(self, v, n, t="NULL"):
        ...
        if t == "NULL":
            ...
            self.normal_lighting = Vec3(
                (normalized_L.dot(self.n1) * 255, normalized_L.dot(self.n2) * 255, normalized_L.dot(self.n3) * 255)
            )
    ...
    def Shade_p(self, point, shader, color="NULL", diffuse=Vec3((0, 0, 0))):
        ...
        # print(shader)
        avg = (
            (mixer[0] * self.normal_lighting.z)
            + (mixer[1] * self.normal_lighting.y)
            + (self.normal_lighting.x * mixer[2])
        )
        color_r = 1 - shader
